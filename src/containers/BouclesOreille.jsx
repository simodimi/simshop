...
const BouclesOreille = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...bouclesoreilles]); //etat initial
  const min = Math.min(...bouclesoreilles.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...bouclesoreilles.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = bouclesoreilles.filter(
