...
const Chaussures = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...chaussures]); //etat initial
  const min = Math.min(...chaussures.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...chaussures.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = chaussures.filter(
          ...
          {showfilter && (
            <div className="Filter-Home-before">
              ...
              <div className="Filter-Home-Before-down">
                ...
                {/* Prop 'intervalle' est passée à FunctionPrixchaussures pour gérer les valeurs d'intervalle de prix */}
      ...
      <div className="Home-before-Selection">
        {sortOrder.map((item) => (
          <div className="Home-before-Item" key={item.id}>
            <Link to={`/shop/chaussure/${item.id}`}>
