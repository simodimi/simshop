...
import { robes } from "../components/data";
...
const Robes = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...robes]); //etat initial
  const min = Math.min(...robes.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...robes.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = robes.filter(
