...
import { television } from "../components/data";
...
const Tv = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...television]); //etat initial
  const min = Math.min(...television.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...television.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = television.filter(
