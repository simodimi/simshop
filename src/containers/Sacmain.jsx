...
const Sacmain = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...sacmain]); //etat initial
  const min = Math.min(...sacmain.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...sacmain.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = sacmain.filter(
