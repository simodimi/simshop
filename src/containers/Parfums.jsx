...
const Parfums = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...parfums]); //etat initial
  const min = Math.min(...parfums.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...parfums.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = parfums.filter(
