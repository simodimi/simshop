...
import { gants } from "../components/data";
...
const Gants = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...gants]); //etat initial
  const min = Math.min(...gants.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...gants.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = gants.filter(
