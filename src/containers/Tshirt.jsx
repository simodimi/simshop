...
const Tshirt = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...tshirts]); //etat initial
  const min = Math.min(...tshirts.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...tshirts.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = tshirts.filter(
