...
const Vestes = ({ addshop }) => {
  ...
  const [sortOrder, setSortOrder] = useState([...vestes]); //etat initial
  const min = Math.min(...vestes.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...vestes.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = vestes.filter(
