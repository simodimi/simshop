...
import { pantalons } from "../components/data";
...
const Pantalons = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...pantalons]); //etat initial
  const min = Math.min(...pantalons.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...pantalons.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = pantalons.filter(
