...
const Robots = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...robots]); //etat initial
  const min = Math.min(...robots.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...robots.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = robots.filter(
