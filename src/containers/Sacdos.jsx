...
import { sacdos } from "../components/data";
...
const Sacdos = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...sacdos]); //etat initial
  const min = Math.min(...sacdos.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...sacdos.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = sacdos.filter(
