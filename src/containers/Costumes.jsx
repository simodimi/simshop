...
import { costumes } from "../components/data";
...
const Costumes = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...costumes]); //etat initial
  const min = Math.min(...costumes.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...costumes.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = costumes.filter(
