...
import { appareilelectroniques } from "../components/data";
import { FcFilledFilter } from "react-icons/fc";
import FunctionPrixTelephone from "../function/FunctionPrixTelephone";

const AppareilElectronique = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...appareilelectroniques]); //etat initial
  const min = Math.min(...appareilelectroniques.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...appareilelectroniques.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = appareilelectroniques.filter(
...
export default AppareilElectronique;
