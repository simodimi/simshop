...
const Sport = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...sport]); //etat initial
  const min = Math.min(...sport.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...sport.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = sport.filter(
