...
const Cravattes = () => {
  ...
  const [sortOrder, setSortOrder] = useState([...cravattes]); //etat initial
  const min = Math.min(...cravattes.map((p) => p.prix)); //prix le plus bas du tableau
  const max = Math.max(...cravattes.map((p) => p.prix)); //prix le plus haut du tableau
  ...
  const applyFilterAndSort = (sortType, priceRange) => {
    const filteredItems = cravattes.filter(
